...
LLDSPEC gBool gdisp_lld_init(GDisplay* g)
{
    ...
    write_data(g,
        0x06);           // Horizontal address increment,Disable Column Address Re-map,Enable Nibble
                         // Re-map,Scan from COM[N-1] to COM0,Disable COM Split Odd Even
    ...
    write_data(g,
        0x005 | 0x0fd);  //	0xfd,Enhanced low GS display quality;default is 0xb5(normal),

    write_cmd(g, CMD_SET_CONTRAST_CURRENT);
    write_data(g, 0x9f); // 0xff 	default is 0x7f

    write_cmd(g, CMD_MASTER_CURRENT_CONTROL);
    write_data(g, 0x0a); // default is 0x0f
...
#else
    const gU16 bufferSize = GDISP_SCREEN_HEIGHT * (GDISP_SCREEN_WIDTH / 2);
    for (gU16 index = 0; index < bufferSize; ++index)
    {
        write_data(g, ram[index]);
    }
